...
class PoliceClearanceCertificateViewPage extends StatefulWidget {
  const PoliceClearanceCertificateViewPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _PoliceClearanceCertificateViewPageState createState() =>
      _PoliceClearanceCertificateViewPageState();
}

class _PoliceClearanceCertificateViewPageState
    extends State<PoliceClearanceCertificateViewPage> {
  ...
  Future<void> fetchPccData() async {
    ...
    try {
      ...
      const fetchPccUrl =
          '$baseUrl/androidapi/mobile/service/getApplicationStatusOfPcr';
  ...
  Future<void> _openView(String reqno) async {
    ...
    try {
      ...
      final pdfUrl =
          '$baseUrl/androidapi/mobile/service/viewCharCertApplicationDetailsforpcr';
      final pccPdfResponse = await client.post(
        ...
        body: jsonEncode({"uid": "$loginId", "reqno": reqno}),
      );
  ...
  Future<void> _downloadPdf(String reqno) async {
    ...
    try {
      ...
      final client = IOClient(ioc);
        ...
        final pdfUrl = '$baseUrl/androidapi/mobile/service/printPcc?pcc=$reqno';
        ...
        if (pdfResponse.statusCode == 200) {
          if (pdfResponse.bodyBytes.isNotEmpty) {
            ...
            final filePath =
                '$directory/Police_Clearance_Certificate_$reqno.pdf';
